...
4. **After the meeting, learn from it.** Scribe joins and records on its own — you don't dispatch it. Once the meeting is over, pull the transcript + insights via the Scribe tools (`mcp__scribe__list_meetings` → `get_transcript` / `get_insights`): attendees, decisions, who said what. From that, update the account: the success plan and next steps, commitments with owners, blockers, and a health signal. Scribe already summarises each meeting for the attendees — do not write one yourself.
