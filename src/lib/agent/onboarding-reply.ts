...
export function buildReplyFramingPrompt(args: {
  ...
  return [
    ...
    "",
    "   **Set `start_clock: true` on any customer-side objective with a due date.**",
    "   The customer has already been asked — they told you about this in the reply,",
    "   and their own date is the commitment. Waiting for a further outbound ask",
    "   before starting the clock means nothing chases it until somebody notices.",
    "",
    "   Do not write the chase into the description. \"Chase them if not confirmed by",
    "   Friday\" in prose is an instruction to nobody: no code reads that field. The",
    "   follow-up mechanism is `next_followup_at`, which `start_clock` sets. An",
    "   intention recorded as text instead of as state is the same failure as a",
    "   safety rule recorded as a prompt instead of as an absent tool.",
    "",
